...
public class Sound
{
    ...
    bool _PlayBgm(string key, float volume = 1.0f)
    {
        ...
        // サウンドボリューム設定
        if (_soundOnOff) source.volume = volume;
    ...
    /// SEの再生
    /// ※事前にLoadSeでロードしておくこと
    public static bool PlaySe(string key, int channel = -1, float volume = 1.0f, bool isLoop = false)
    {
        return GetInstance()._PlaySe(key, channel, volume, isLoop);
    }
    bool _PlaySe(string key, int channel = -1, float volume = 1.0f, bool isLoop = false)
    {
    ...
    /// SEの停止
    public static bool StopSe(bool isAll = false)
    {
        if (isAll) return GetInstance()._StopAllSe();
        else return GetInstance()._StopSe();
    }
    bool _StopSe()
    {
    ...
    public static bool StopSeChannel(int channel)
    {
        return GetInstance()._StopSeChannel(channel);
    }
    bool _StopSeChannel(int channel)
    {
    ...
    {
        GetInstance()._SetSoundSetting();
    }
    void _SetSoundSetting()
    {
        _soundOnOff = SaveData.GetBool("soundOnOff", true);
    }
}